        ...
        <div class="p-4">
          <Form @submit="login" class="needs-validation">
            ...
            <div :class="`mb-4 text-${themeText}`">
              <ErrorMessage
                :class="`note note-danger bg-${theme}`"
                name="email"
              />
            </div>
            ...
            >
              ...
              <label :class="`form-label text-${themeText}`" for="form3Example3"
                >{{ $t("view.login.user.password") }}*</label
              >
            </div>
            <div :class="`mb-4 text-${themeText}`">
              <ErrorMessage
  ...
  methods: {
    ...
    async login(data) {
      try {
        ...
        this.$toast.error(
          error.response.data?.msg || this.$t("view.login.error")
        );
