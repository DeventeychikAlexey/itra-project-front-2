...
export default {
  ...
  async UPGRADE_USER({}, { id, right }) {
    const resp = await axios.put(
      `https://itra-project.herokuapp.com/back/${right}/upgradeUser/${id}`
    );
    return resp.data.msg;
  },
  async DOWNGRADE_USER({}, { id, right }) {
    const resp = await axios.put(
      `https://itra-project.herokuapp.com/back/${right}/downgradeUser/${id}`
    );
    return resp.data.msg;
  },
  async TOGGLE_BLOCKED({}, { id, right }) {
    const resp = await axios.put(
      `https://itra-project.herokuapp.com/back/${right}/toggleBlocked/${id}`
    );
    return resp.data.msg;
  },
  async DELETE_USER({}, { id, right }) {
    const resp = await axios.delete(
      `https://itra-project.herokuapp.com/back/${right}/user/${id}`
    );
    return resp.data.msg;
  },
  // Collections
